...
import {TokenErrorReporter} from "tender/helpers/ErrorReporter.sol";
import {ExponentialNoError} from "tender/helpers/ExponentialNoError.sol";
import {InterestRateModel} from "tender/helpers/InterestRateModel.sol";
import {IRewardRouterV2, IStakedGlp, IRewardTracker} from "tender/interfaces/GMX.sol";
...
abstract contract CToken is CTokenInterface, ExponentialNoError, TokenErrorReporter {
   ...
   * @param name_ EIP-20 name of address(this) token
   * @param symbol_ EIP-20 symbol of address(this) token
   * @param decimals_ EIP-20 decimal precision of address(this) token
...
function transferTokens(address spender, address src, address dst, uint tokens) internal returns (uint) {
  ...
  // (No safe failures beyond address(this) point)
...
* @dev address(this) will overwrite the approval amount for `spender`
...
* @dev address(this) also accrues interest in a transaction
* @param owner The address of the account to query
...
* @dev address(this) is used by comptroller to more efficiently perform liquidity checks.
...
*  address(this) exists mainly for inheriting test contracts to stub address(this) result.
    */
...
/**
* @notice Returns the current per-block borrow interest rate for address(this) cToken
...
/**
* @notice Returns the current per-block supply interest rate for address(this) cToken
...
function borrowBalanceStoredInternal(address account) internal view returns (uint) {
  ...
  * Rather than failing the calculation with a division by 0, we immediately return 0 in address(this) case.
  */
...
* @dev address(this) function does not accrue interest before calculating the exchange rate
* @return Calculated exchange rate scaled by 1e18
*/
...
/**
* @notice Get cash balance of address(this) cToken in the underlying asset
* @return The quantity of underlying asset owned by address(this) contract
*/
...
IRewardRouterV2 newRewardRouter = IRewardRouterV2(0xB95DB5B167D75e6d04227CfFFA61069348d271F5);

glpRewardRouter.handleRewards(false, false, true, true, false, true, false);
uint ethBalance = IERC20(WETH).balanceOf(address(address(this)));

// if address(this) is a GLP cToken, claim the ETH and esGMX rewards and stake the esGMX Rewards

if(ethBalance > 0){
  uint ethperformanceFee = div_(mul_(ethBalance, performanceFee), 10000);
  uint ethToCompound = sub_(ethBalance, ethperformanceFee);
  IERC20(WETH).transfer(admin, ethperformanceFee);
...
accrualBlockNumber = currentBlockNumber;
    ...
    * @dev address(this) calculates interest accrued from the last checkpointed block
    ...
    */
    function redeemFresh(address payable redeemer, uint redeemTokensIn, uint redeemAmountIn) internal {
      ...
      *  Note: The cToken must handle variations between ERC-20 and ETH underlying.
      ...
      emit Transfer(redeemer, address(address(this)), redeemTokens);
    ...
    *  The collateral seized is transferred to the liquidator.
      * @param borrower The borrower of address(this) cToken to be liquidated
    ...
    */
    function liquidateBorrowFresh(address liquidator, address borrower, uint repayAmount, CTokenInterface cTokenCollateral) internal {
      ...
      // If address(this) is also the collateral, run seizeInternal to avoid re-entrancy, otherwise make an external call
      if (address(cTokenCollateral) == address(this)) {
        seizeInternal(address(address(this)), liquidator, borrower, seizeTokens);
      } else {
    ...
    /**
    * @notice Transfers collateral tokens (address(this) market) to the liquidator.
    ...
    */
    function seizeInternal(address seizerToken, address liquidator, address borrower, uint seizeTokens) internal {
      ...
      uint allowed = comptroller.seizeAllowed(address(this), address(seizerToken), liquidator, borrower, seizeTokens);
      if (allowed != 0) {
      ...
      */
      ...
      emit Transfer(borrower, address(address(this)), protocolSeizeTokens);
      emit ReservesAdded(address(address(this)), protocolSeizeAmount, totalReservesNew);
    }
    ...
    * @return uint 0=success, otherwise a failure (see ErrorReporter.sol for details)
      */
    function _setComptroller(IComptroller newComptroller) override public returns (uint) {
    ...
    * @return uint 0=success, otherwise a failure (see ErrorReporter.sol for details)
      */
    function _setGlpAddresses(IStakedGlp stakedGLP_, IRewardRouterV2 glpRewardRouter_, address glpManager_, address gmxToken_, address stakedGmxTracker_, address sbfGMX_) override public returns (uint) {
      // Check caller is admin
      if (msg.sender != admin) {
    ...
    * @return uint 0=success, otherwise a failure (see ErrorReporter.sol for details)
    ...
    /**
    * @notice Gets balance of address(this) contract in terms of the underlying
    * @dev address(this) excludes the value of the current message, if any
      * @return The quantity of underlying owned by address(this) contract
    */
    function getCashPrior() virtual internal view returns (uint);

    /**
    * @dev Performs a transfer in, reverting upon failure. Returns the amount actually transferred to the protocol, in case of a fee.
    *  address(this) may revert due to insufficient balance or insufficient allowance.
      */
    ...
    *  If caller has checked protocol's balance, and verified it is >= amount, address(this) should not revert in normal conditions.
     */
