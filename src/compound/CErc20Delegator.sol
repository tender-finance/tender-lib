...
import {CErc20Interface, CDelegatorInterface, CTokenInterface} from './CTokenInterfaces.sol';
import {IRewardRouterV2, IStakedGlp, IRewardTracker} from "tender/interfaces/GMX.sol";
import {IERC721Receiver, IERC721} from 'tender/interfaces/Tokens.sol';
...
contract TenderDelegator is CTokenInterface, CErc20Interface, CDelegatorInterface {
  ...
  */
  function _setImplementation(address implementation_, bool allowResign, bytes memory becomeImplementationData)override public {
    allowResign; // Shh, unused variable
    becomeImplementationData; // Shh, unused variable
  ...
  */
  function delegateTo(address callee, bytes memory data) internal returns (bytes memory) {
    callee;
    (bool success, bytes memory returnData) = implementation.call(data);
    assembly {
      if eq(success, 0) {
  ...
  */
  function delegateToViewImplementation(bytes memory data) public view returns (bytes memory) {
    (bool success, bytes memory returnData) = implementation.staticcall(abi.encodeWithSignature("delegateToImplementation(bytes)", data));
    assembly {
      if eq(success, 0) {
        revert(add(returnData, 0x20), returndatasize())
      }
